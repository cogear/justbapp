import {
    Body,
    Container,
    Head,
    Heading,
    Html,
    Link,
    Preview,
    Section,
    Text,
    Tailwind,
} from '@react-email/components';
import * as React from 'react';

export const DirectMessageEmail = ({
    senderName = 'A member',
    messageText = '',
    conversationUrl = 'https://theblife.com/messages',
}: {
    senderName?: string;
    messageText?: string;
    conversationUrl?: string;
}) => (
    <Html>
        <Head />
        <Preview>{`${senderName} sent you a message on The B Life`}</Preview>
        <Tailwind
            config={{
                theme: {
                    extend: {
                        colors: {
                            primary: '#A67C52',
                            background: '#FDFCFB',
                        },
                        fontFamily: {
                            serif: ['Georgia', 'serif'],
                        },
                    },
                },
            }}
        >
            <Body className="bg-white my-auto mx-auto font-sans">
                <Container className="border border-solid border-[#eaeaea] rounded my-[40px] mx-auto p-[20px] w-[465px]">
                    <Section className="mt-[32px]">
                        <Heading className="text-black text-[20px] font-serif font-normal text-center p-0 my-[24px] mx-auto">
                            {senderName} sent you a message
                        </Heading>
                    </Section>
                    <Section className="bg-[#FDFCFB] border border-solid border-[#eaeaea] rounded p-[16px]">
                        <Text className="text-black text-[14px] leading-[24px] whitespace-pre-wrap m-0">
                            {messageText}
                        </Text>
                    </Section>
                    <Text className="text-[#666666] text-[13px] leading-[22px] text-center mt-[24px]">
                        Reply to this email to respond, or view the conversation online.
                    </Text>
                    <Section className="text-center mt-[16px] mb-[32px]">
                        <Link
                            href={conversationUrl}
                            className="bg-black rounded text-white text-[12px] font-semibold no-underline text-center px-5 py-3"
                        >
                            View conversation
                        </Link>
                    </Section>
                    <Section className="border-t border-solid border-[#eaeaea] my-[16px] mx-0 w-full" />
                    <Text className="text-[#666666] text-[12px] leading-[20px] text-center">
                        Sent through The B Life — your email address stays private.
                        <br />
                        theblife.com • breathe … you&rsquo;re here
                    </Text>
                </Container>
            </Body>
        </Tailwind>
    </Html>
);

export default DirectMessageEmail;
