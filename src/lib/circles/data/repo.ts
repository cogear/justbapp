// Circles data-access — the ONLY circles module that touches Prisma.
...
import type {
  CirclesGroup as GroupRow,
  CirclesMember as MemberRow,
  CirclesMeetup as MeetupRow,
  CirclesRsvp as RsvpRow,
  CirclesDecision as DecisionRow,
  CirclesVote as VoteRow,
  CirclesPhoto as PhotoRow,
} from '@prisma/client';
...
export async function createGroup(input: CreateGroupInput): Promise<core.Group> {
  ...
  const row = await prisma.$transaction(async (tx) => {
    const group = await tx.circlesGroup.create({
      data: {
        name: input.name,
        kind: input.kind,
...
export async function inviteMember(groupId: string, userId: string): Promise<core.Member> {
  const row = await prisma.circlesMember.upsert({
...
export async function createMeetup(
  ...
  const row = await prisma.circlesMeetup.upsert({
...
export async function setRsvp(
  ...
  const row = await prisma.circlesRsvp.upsert({
...
export async function getMeetupView(meetupId: string): Promise<MeetupView | null> {
  const row = await prisma.circlesMeetup.findUnique({
    where: { id: meetupId },
...
export async function listGroupsForGeneration(): Promise<core.Group[]> {
  const rows = await prisma.circlesGroup.findMany();
...
export async function findPreMeetupCandidates(
  ...
  const rows = await prisma.circlesMeetup.findMany({
...
export async function recordReminderSent(
  ...
  try {
    await prisma.circlesReminder.create({ data: { meetupId, userId, kind } });
  } catch (e) {
...
export async function createDecision(
  ...
  const row = await prisma.$transaction(async (tx) => {
    const decision = await tx.circlesDecision.create({
      data: { meetupId, type, options: options as unknown as object, closesAt },
    });
    await tx.circlesMeetup.update({ where: { id: meetupId }, data: { status: 'deciding' } });
    return decision;
  });
...
export async function castVote(
  ...
  const decision = await prisma.circlesDecision.findUnique({ where: { id: decisionId } });
  ...
  const row = await prisma.circlesVote.upsert({
...
export async function getDecisionView(decisionId: string): Promise<DecisionView | null> {
  const row = await prisma.circlesDecision.findUnique({
    where: { id: decisionId },
    include: { votes: true },
  });
  if (!row) return null;
...
export async function listDueDecisions(now: Date): Promise<core.Decision[]> {
  const rows = await prisma.circlesDecision.findMany({
...
export async function getVotes(decisionId: string): Promise<core.Vote[]> {
  const rows = await prisma.circlesVote.findMany({ where: { decisionId } });
...
export async function resolveDecision(
  ...
  await prisma.$transaction(async (tx) => {
    const decision = await tx.circlesDecision.update({
      where: { id: decisionId },
      data: { status: 'resolved', resolvedOptionId: winningOptionId },
    });
    await tx.circlesMeetup.update({
...
export async function setMemberRole(
  ...
  const row = await prisma.circlesMember.update({
    where: { groupId_userId: { groupId, userId } },
    data: { role },
  });
  return toMember(row);
...
export async function markPastMeetupsDone(now: Date): Promise<number> {
  const res = await prisma.circlesMeetup.updateMany({
...
export async function addPhoto(
  ...
  const row = await prisma.circlesPhoto.create({
...
export async function listPhotos(meetupId: string): Promise<core.Photo[]> {
  const rows = await prisma.circlesPhoto.findMany({
    where: { meetupId },
