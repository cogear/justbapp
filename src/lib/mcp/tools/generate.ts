...
import { principles } from '../data/book-content';
import { phantomPersonas } from '../data/phantom-personas';
...
${principle.quotes.map((q: string) => `- "${q}"`).join('\n')}
...
export async function generateAiNewsPost() {
    const articles = await prisma.newsArticle.findMany({
        ...
        select: { id: true, title: true, description: true, source: true, publishedAt: true }
    });

    if (articles.length === 0) {
        return { success: false, message: "No news articles found. Run news ingestion first." };
    }
    ...
    return {
        ...
        suggested_author: { name: persona.name, email: persona.email, personality: persona.personality },
        suggested_space: "general",
...
Recent AI news:
${articles.map((a: any) => `- "${a.title}" (${a.source}): ${a.description || 'No description'}`).join('\n')}
