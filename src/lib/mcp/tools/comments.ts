...
export async function createComment(args: { content: string; post_id: string; author_email: string }) {
    ...
    if (!post) return { success: false, message: `Post "${args.post_id}" not found` };

    const user = await prisma.user.findUnique({ where: { email: args.author_email } });
    if (!user) return { success: false, message: `User "${args.author_email}" not found` };

    const comment = await prisma.comment.create({
        data: { content: args.content, postId: args.post_id, authorId: user.id },
        include: { author: true }
    });
