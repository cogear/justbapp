// Gatherings engine — in-process API surface. Orchestrates core + data; session-less;
...
// cron) may import `@/lib/gatherings/service`, but still NOT `core` or `data`
...
export async function generateDueMeetups(now: Date = new Date()): Promise<GenerationResult[]> {
  ...
  for (const g of groups) {
    ...
    try {
      ...
      console.error(`[gatherings] generation skipped group ${g.id}:`, e);
...
/**
 * Generate one gathering's upcoming occurrences from its cadence, on demand.
 * Idempotent. A no-op for adhoc (single/flexible) gatherings, whose dates are
 * set by hand. Used by the in-app "Generate upcoming" button.
 */
export async function generateForGroup(
  ...
  const instants = core.expandCadence(group.defaultCadence, group.timezone, now, core.MEETUP_HORIZON_DAYS);
  for (const startsAt of instants) {
    const rsvpCutoffAt = new Date(startsAt.getTime() - core.RSVP_CUTOFF_LEAD_HOURS * 3600_000);
    await repo.createMeetup(groupId, startsAt, { rsvpCutoffAt });
  }
  return { created: instants.length };
}
...
/** Delete an occurrence (and everything attached). Requires organizer rights. */
export async function deleteMeetup(meetupId: string, actingUserId: string): Promise<void> {
  const ctx = await repo.getMeetupWithGroup(meetupId);
  if (!ctx) throw new Error('meetup not found');
  await assertOrganizer(ctx.meetup.groupId, actingUserId);
  await repo.deleteMeetup(meetupId);
}

/** Set a meetup's location. Requires organizer rights. */
export async function setLocation(
  ...
  if (!ctx) throw new Error('meetup not found');
  await assertOrganizer(ctx.meetup.groupId, actingUserId);
  if (!locationText?.trim()) throw new Error('location is required');
  return repo.setMeetupLocation(meetupId, locationText.trim(), placeId);
}
