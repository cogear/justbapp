...
async function cleanupGroup(groupId: string) {
  ...
  if (meetupIds.length) {
    const decisions = await prisma.gatheringsDecision.findMany({
      where: { meetupId: { in: meetupIds } },
      select: { id: true },
    });
    const decisionIds = decisions.map((d) => d.id);
    if (decisionIds.length) {
      await prisma.gatheringsVote.deleteMany({ where: { decisionId: { in: decisionIds } } });
      await prisma.gatheringsDecision.deleteMany({ where: { id: { in: decisionIds } } });
    }
...
describe('gatherings agent surface (integration)', () => {
