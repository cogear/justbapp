# Gatherings — Phase 0 Implementation Plan

Companion to [`gatherings-architecture-spec.md`](./gatherings-architecture-spec.md). This is the concrete build plan for **Phase 0 only** — the skeleton + seam + one vertical slice — written against this repo's actual patterns (Prisma 5.22 default-export client, Stack auth, server actions for in-app DB access, route handlers for external/MCP).
...
**Done when:** the slice runs green through an automated test, the in-app path uses server actions, the external path uses `/api/gatherings/*` route handlers, both delegate to the same pure core + data module, and a dependency-lint rule fails the build if host code imports `gatherings/core`.
...
src/lib/gatherings/
  core/                 pure TS — types + rule functions, ZERO io, ZERO next, ZERO prisma
    ...
    index.ts            re-exports
  data/                 the ONLY place that touches Prisma for gatherings_* tables
...
### 1. Pure core — `src/lib/gatherings/core/`
...
Add models with **`@@map`** to get the `gatherings_` table prefix (existing models have no `@@map`, so this is a deliberate, isolated convention for gatherings). **No relation/FK to `User`** — `userId`/`ownerUserId` are bare `String`. Comment loudly.

```prisma
/// OPAQUE host id (DB User.id). DELIBERATELY no relation/FK to User — do not "fix" this.
model GatheringsGroup {
  ...
  members        GatheringsMember[]
  meetups        GatheringsMeetup[]
  @@map("gatherings_groups")
}

model GatheringsMember {
  id        String       @id @default(uuid())
  groupId   String
  ...
  group     GatheringsGroup  @relation(fields: [groupId], references: [id])
  @@map("gatherings_members")
}

model GatheringsMeetup {
  id           String        @id @default(uuid())
  groupId      String
  startsAt     DateTime
  locationText String?
  ...
  decisionId   String?
  group        GatheringsGroup   @relation(fields: [groupId], references: [id])
  rsvps        GatheringsRsvp[]
  @@unique([groupId, startsAt])   // idempotent meetup generation
  @@map("gatherings_meetups")
}

model GatheringsRsvp {
  id        String        @id @default(uuid())
  meetupId  String
  userId    String        /// opaque User.id, no FK
  state     String         @default("none")
  updatedAt DateTime       @updatedAt
  meetup    GatheringsMeetup  @relation(fields: [meetupId], references: [id])
  @@unique([meetupId, userId])    // one rsvp row per user per meetup
  @@map("gatherings_rsvps")
}
```
...
Relations *between* gatherings models (group↔member↔meetup↔rsvp) are fine and good — the rule is only "no FK out of the gatherings namespace into theblife tables." Then sync with **`npx prisma db push`** (NOT `migrate dev`): the repo has no `prisma/migrations/` dir and is managed by `db push`; running `migrate dev` against a push-managed DB detects "drift" and offers a destructive reset. `db push` applies the additive `gatherings_*` tables non-destructively (it aborts rather than dropping data).

### 3. Data module — `src/lib/gatherings/data/repo.ts`

The single Prisma-touching module for gatherings. Imports `@/lib/prisma` and `core`; maps rows ↔ entities; runs core rules in the middle.
...
export async function createGroup(input: {...}): Promise<core.Group> {
  ...
  const row = await prisma.gatheringsGroup.create({ data: {...} });
  // also create the owner GatheringsMember (role owner, status active)
  return toGroup(row);
}
...
export async function setRsvp(meetupId: string, userId: string, state: core.RsvpState) {
  return prisma.gatheringsRsvp.upsert({ where: { meetupId_userId: { meetupId, userId } }, ... });
}
export async function getMeetupView(meetupId: string) {                       // returns headcount + rsvps
  const rsvps = await prisma.gatheringsRsvp.findMany({ where: { meetupId } });
...
`getOrCreateUser()` currently lives privately in `src/app/community/actions.ts:7`. Lift it verbatim into `@/lib/auth` so both that file and Gatherings use one copy. The canonical opaque token is `user.id` (the DB `User.id` uuid). Update `community/actions.ts` to import it (small, safe refactor).

### 5. In-app server actions — `src/app/gatherings/actions.ts`
...
### 6. External route handlers — `src/app/api/gatherings/*`
...
```ts
// src/app/api/gatherings/meetups/[id]/rsvp/route.ts
...
- A global rule: `no-restricted-imports` banning `@/lib/gatherings/core` and `@/lib/gatherings/data` everywhere…
- …re-enabled (override turns the rule off) **only** for files under `src/lib/gatherings/**`, `src/app/gatherings/**`, `src/app/api/gatherings/**`.

Result: if any host component or unrelated action imports gatherings internals, `npm run lint` (and `build`) fails. Add a one-line comment pointing back to `ARCHITECTURE.md`. The companion "host must not read `gatherings_*` tables" rule is weaker to lint mechanically; enforce by convention — `prisma.gatherings*` calls live only in `data/repo.ts` — and note it in `ARCHITECTURE.md` for review.
...
- **(A) Vitest + a real test Postgres** (recommended). Add `vitest` dev-dep and a `test` script; point `DATABASE_URL`/`DIRECT_URL` at a scratch DB (local Docker Postgres or a Neon/Supabase branch). Test drives the slice through `repo.*` (or the server actions), asserts `headcount.yes === 1`, truncates `gatherings_*` between runs. ~Most faithful, reusable for later phases.
- **(B) A `tsx` script** (`scripts/gatherings-smoke.ts`, like the existing `seed:courses`) that runs the slice against a scratch DB and asserts. Zero new framework, lighter, but not a real test runner.
...
### 9. `ARCHITECTURE.md` in `src/lib/gatherings/`
...
- `npm run lint` passes; add a throwaway `import ... from '@/lib/gatherings/core'` in a host file and confirm lint **fails**, then delete it (proves the seam bites).
- `npx prisma db push` applies; `gatherings_*` tables exist; an information_schema check shows **no FK** from `gatherings_*` to `User` (only gatherings→gatherings FKs).
- Integration test green: the full slice yields headcount `{ yes: 1 }`.
- Manual external-path check (optional): `curl` the `/api/gatherings/*` endpoints with the `MCP_API_KEY` bearer to confirm the same slice works over HTTP.
