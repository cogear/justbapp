# Gatherings — Recurring Group Engine
...
Working name: **Gatherings**. Swap freely — the code uses `gatherings` as a namespace, not a brand.
...
theblife.com talks to the engine through a thin service surface (server actions in-app, plus `/api/gatherings/*` and MCP for external callers). **It never imports the engine's domain code and never touches the engine's tables directly.**

That single constraint is the whole game. The day Gatherings earns its own life, you change *who calls the service* — not the engine.

---

## 2. Package / deploy shape

The three *logical* layers below are real and enforced. In v1 they live as **directories inside the existing flat Next.js app**, not as separate npm packages — the repo has no workspaces/Turborepo today, and standing up a monorepo just to draw a package boundary is extraction tax you haven't earned. The seam is enforced by a **dependency-lint rule** (host code may not import from `gatherings/core`), which is as binding as a package boundary and far cheaper. Promote `gatherings/core` to a real `@blife/gatherings-core` workspace package *at the extraction checkpoint* (§5), not before.

```
theblife.com (host app — Next.js on Vercel)
│
├─ provides: auth, distribution, brand framing, the b. life UI
├─ calls: gatherings via server actions (in-app) + /api/gatherings/* (MCP/external)
└─ NEVER imports gatherings/core, NEVER reads gatherings_* tables directly
        │
        ▼  (the seam — enforced by dependency-lint, not a package boundary)
        │
gatherings service  (src/app/gatherings/actions.ts + src/app/api/gatherings/* + data module)
│
├─ server actions       in-app mutations (RSVP from a click — the app's idiom)
├─ /api/gatherings/*        route handlers mirroring the same ops for MCP/external
├─ /api/gatherings/mcp      MCP tools (decision pending — see §4)
├─ owns tables:          gatherings_*   (prefixed, in the public schema for v1)
└─ depends on:           src/lib/gatherings/core
        │
        ▼
src/lib/gatherings/core   (pure domain — the crown jewel)
│
...
All Gatherings tables carry a **`gatherings_` name prefix and live in the existing `public` schema** for v1. The repo runs Prisma against a single schema and uses no Postgres schema namespaces anywhere; forcing a dedicated `gatherings` PG schema means Prisma multi-schema config for isolation that's actually enforced by code discipline + opaque ids, not by the schema boundary. The prefix gives you the same "never tangle with theblife.com tables" guarantee at zero config cost. (Promoting `gatherings_*` → a true `gatherings` PG schema is a clean, mechanical move at the extraction checkpoint.)

The tables below are shown by their logical names; the physical Prisma models are `GatheringsGroup`, `GatheringsMember`, etc., mapping to `gatherings_groups`, `gatherings_members`, ….
...
### `gatherings_decisions` (the optional "decide step")
| field | type | notes |
|---|---|---|
| id | uuid pk | |
| meetup_id | uuid fk | |
...
> **Votes-as-jsonb is a v1 shortcut, not the destination.** A jsonb array enforces no "one vote per user" and races on concurrent writes (two voters read-modify-write the same blob). Acceptable for 6–10-person groups where contention is near zero. When groups grow or you need integrity, migrate to a `gatherings_votes` table (row per vote) with `unique(decision_id, user_id)`. Flagged so it's a known trade, not a latent bug.
...
```ts
// src/app/gatherings/actions.ts   'use server'
...
**(b) `/api/gatherings/*` route handlers — the external/future-host seam.** The same operations, re-exposed over HTTP for MCP and any future host that isn't theblife.com. This is the surface that makes extraction real.

```
POST /api/gatherings/groups                 create group
POST /api/gatherings/groups/:id/invite      invite a user_id
POST /api/gatherings/groups/:id/meetups     generate next meetup(s) from cadence
POST /api/gatherings/meetups/:id/rsvp       { user_id, state }
GET  /api/gatherings/meetups/:id            headcount, RSVPs, decision state
POST /api/gatherings/decisions/:id/vote     { user_id, option_id }
```
...
**Phase 3 — history & polish.** "Last time / next time" group timeline. Co-organizers. Discovery within already-connected gatherings. Optional photos/ratings.
...
**Extraction checkpoint (anytime after Phase 2):** if usage shows groups that want it *without* the b. life framing, extract. This is the moment the deferred physical commitments get paid for, mechanically: promote `src/lib/gatherings/core` → a real `@blife/gatherings-core` workspace package; migrate `gatherings_*` tables → a dedicated `gatherings` PG schema (or a separate database); move the action/route layer to its own repo/domain. Because core was kept framework-free and the host only ever reached Gatherings through the action/route seam (enforced by the dependency-lint rule), each of these is a move, not a rewrite.
...
```
We're building a modular recurring-group engine called "Gatherings" inside the
...
Enforce these rules at every step. Flag me if anything would violate them:
  ...
  1. Three LOGICAL layers, living as directories in this flat Next.js app
     ...
     extraction tax we haven't earned yet):
       src/lib/gatherings/core  — pure TS domain. IO-free. NO next.js, NO Prisma/
                               ...
                               business rules.
       gatherings service       — server actions (src/app/gatherings/actions.ts) for
                               in-app mutations PLUS route handlers under
                               /api/gatherings/* for MCP/external. Both delegate to
                               gatherings/core and a single gatherings data module
                               that is the ONLY place touching Prisma.
       host (theblife.com)   — reaches Gatherings ONLY through that action/route
                               layer. It must NEVER import gatherings/core and
                               NEVER read gatherings_* tables directly. Add a
  ...
  3. Database: Prisma models prefixed `Gatherings*` → tables `gatherings_*` in the
     existing `public` schema (we use no PG schema namespaces today). Never FK
     from gatherings_* tables into theblife.com tables. (A dedicated `gatherings` PG
...
Start with PHASE 0 only — do not build ahead:
  ...
  - Scaffold src/lib/gatherings/core with entities Group, Member, Cadence,
    Meetup, RSVP, Decision (types + a few rule functions: addMember,
    rsvp(meetup, user, state), headcount(meetup)). Pure functions, unit
    tested, no IO.
  - Add the `gatherings_*` Prisma models for the tables in the spec (groups with
  ...
  - Implement the vertical slice. In-app path = server actions; also expose
    the matching route handlers for the external seam:
        POST /api/gatherings/groups
        POST /api/gatherings/groups/:id/invite
        POST /api/gatherings/groups/:id/meetups   (single meetup for now)
        POST /api/gatherings/meetups/:id/rsvp
        GET  /api/gatherings/meetups/:id          (returns headcount + rsvps)
  ...
  - Add a short ARCHITECTURE.md in the gatherings folder restating the 3-layer
